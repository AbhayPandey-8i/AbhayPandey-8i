...
const USERNAME = process.env.USERNAME || 'AbhayPandey-8i';
...
function calcStreak(commitDays) {
  ...
  // so "today" matches your local date
  ...
  return {
    cur,
    longest,
    total: Object.values(commitDays)
      .reduce((a, b) => a + b, 0),
    sStart: fmt(sStart),
    sEnd: fmt(sEnd),
...
async function main() {
  ...
  // Build commitDays from GraphQL calendar
  const commitDays = {};

  const calendar =
    graphqlRes?.data?.user?.contributionsCollection
      ?.contributionCalendar;
  ...
  console.log(
    `GraphQL: ${totalContributions} total contributions this year`
  );

  console.log(
    `Active days found: ${Object.keys(commitDays).length}`
  );
